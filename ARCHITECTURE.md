...
│  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  │   │
│  │  │   Memory     │  │  PostgreSQL  │  │   MySQL 8.0+ │  │   │
│  │  │ (开发/测试)  │  │  (生产推荐)  │  │  (生产可选)  │  │   │
│  │  └──────────────┘  └──────────────┘  └──────────────┘  │   │
│  │  - Session 持久化 (会话元数据)                          │   │
│  │  - Event 持久化 (消息历史、工具调用)                    │   │
│  │  - JSON 支持 (JSONB / JSON 类型)                        │   │
│  │  - 高性能查询 (索引优化、批量操作)                      │   │
│  └─────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────┘
```
...
### 1. Agent 类型 (`pkg/agent/`)

AgentSDK 提供三种 Agent 类型：

#### Basic Agent
标准的单 Agent 模式，用于执行独立任务。

#### Workflow Agent (Phase 7)
编排多个子 Agent 的工作流模式：
...
### 2. Session Store (`pkg/session/`) - Phase 7
