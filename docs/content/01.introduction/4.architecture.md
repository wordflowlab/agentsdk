---
title: 架构设计
description: 深入理解AgentSDK的三层架构设计
navigation:
...
AgentSDK采用灵感来自[DeepAgents](https://github.com/anthropics/deepagents)的三层可扩展架构，通过清晰的分层和抽象实现高度可扩展性。

## 🏗️ 整体架构

![架构概览](/agentsdk/images/architecture-overview.svg)
...
中间件系统是AgentSDK的核心设计，采用洋葱模型实现功能的灵活组合。

![中间件洋葱模型](/agentsdk/images/middleware-onion.svg)
...
- **优先级**：100
- **工具**：`fs_read`, `fs_write`, `fs_edit`, `fs_ls`, `fs_glob`, `fs_grep`
- **特性**：
  - 自动驱逐大结果（>20k tokens）
  - 系统提示词增强
  - Backend抽象支持
...
AgentSDK采用事件驱动架构，通过三个独立通道分离不同类型的事件。
...
- [示例代码](https://github.com/wordflowlab/agentsdk/tree/main/examples)
- [Phase 6C文档](https://github.com/wordflowlab/agentsdk/blob/main/docs/PHASE6C_MIDDLEWARE_INTEGRATION.md)
