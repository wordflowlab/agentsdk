---
title: 快速开始
description: 5分钟创建你的第一个AgentSDK应用
navigation:
...
3. ✅ 安装AgentSDK：`go get github.com/wordflowlab/agentsdk`
...
func main() {
    ...
    result, err := ag.Chat(context.Background(),
        "请创建一个 hello.txt 文件，内容是 'Hello AgentSDK!'")
    if err != nil {
        log.Fatal(err)
    }
    fmt.Printf("\n\n✅ 完成: %s\n", result.Text)
...
我已经创建了 hello.txt 文件，内容为 'Hello AgentSDK!'
...
| 类别 | 工具名称 | 功能描述 |
|------|---------|---------|
| **文件系统** | `fs_read` | 读取文件内容（支持分页） |
| | `fs_write` | 写入文件 |
| | `fs_edit` | 精确编辑（字符串替换） |
| | `fs_ls` | 列出目录 |
| | `fs_glob` | Glob模式匹配 |
| | `fs_grep` | 正则搜索（显示行号） |
| **命令执行** | `bash_run` | 执行Bash命令 |
...
```go
// 只允许特定工具
Tools: []interface{}{"fs_read", "fs_write"}  // 不包含bash_run
...
for event, _ := range loop.Execute(ctx, "优化代码") {
    fmt.Printf("迭代 %d: %s\n",
...
- [基础Agent](https://github.com/wordflowlab/agentsdk/tree/main/examples/agent)
- [文件系统操作](https://github.com/wordflowlab/agentsdk/tree/main/examples/filesystem)
- [工作流Agent](https://github.com/wordflowlab/agentsdk/tree/main/examples/workflow-agents)
- [Session持久化](https://github.com/wordflowlab/agentsdk/tree/main/examples/session-postgres)
- [MCP集成](https://github.com/wordflowlab/agentsdk/tree/main/examples/mcp)
- [多Agent协作](https://github.com/wordflowlab/agentsdk/tree/main/examples/multi-agent)
- [中间件系统](https://github.com/wordflowlab/agentsdk/tree/main/examples/middleware)
