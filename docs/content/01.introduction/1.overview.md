---
title: 概述
description: 了解AgentSDK的核心价值和设计理念
navigation:
  icon: i-lucide-house
---
...
AgentSDK 是一个为生产环境设计的 **Go语言AI Agent开发框架**。它提供了企业级的Agent运行时，支持事件驱动、云端沙箱、安全可控等特性。
...
AgentSDK采用三通道事件系统，清晰分离不同类型的数据流：
...
- **文件系统**：`fs_read`, `fs_write`, `fs_edit`, `fs_ls`, `fs_glob`, `fs_grep`
- **命令执行**：`bash_run`
- **任务管理**：`todo_list`, `todo_add`, `todo_update`
- **网络请求**：`http_fetch`, `web_search`
...
- **Issues**: [报告问题](https://github.com/wordflowlab/agentsdk/issues)
- **Discussions**: [参与讨论](https://github.com/wordflowlab/agentsdk/discussions)
