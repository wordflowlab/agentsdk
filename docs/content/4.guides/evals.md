...
本示例展示如何使用 `pkg/evals` 对模型输出进行简单的本地评估,对标 Mastra 的 `@mastra/evals` 中的基础 scorer 功能。
...
与 Mastra 的 `@mastra/evals` 对比:

- 相同点:
  - 都提供可组合的 scorer 接口,作为评估 pipeline 的基础构件。
  - 都把“如何存储结果”留给使用方,不强绑定存储层。
- 不同点:
  - 目前 AgentSDK 的 evals 仅提供本地启发式 scorer,不依赖外部 LLM。
  - 后续可以在相同接口下添加:
