...
本示例展示如何使用 `pkg/mcpserver` 将本地工具暴露为一个兼容的 MCP HTTP Server, 对标 Mastra 的 MCP docs server 思路。
...
这与 Mastra 的 `@mastra/mcp-docs-server` 路线一致:  
通过 MCP 将框架/项目/业务知识暴露给更广泛的 Agent 生态使用。
