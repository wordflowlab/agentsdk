...
AgentSDK 提供了一个简单的 CLI 可执行程序 `agentsdk`, 用于快速启动一个标准化的 HTTP Chat 服务, 对标 Mastra 的 `mastra dev` / `@mastra/server` 能力(当前为最小可用版本)。
...
- 扩展 CLI, 支持：
  ...
  - 挂接自定义工具 / 中间件。
  - 启动多 Agent / 多模板实例。
- 与 `pkg/server` 其余扩展能力结合(如 Session API、OpenAPI 生成), 逐步打造一个类似 `mastra dev` 的一站式开发体验。
