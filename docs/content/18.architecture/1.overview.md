...
AgentSDK 采用三层架构设计，将后端核心、HTTP 服务和客户端 SDK 完全解耦：
...
```
依赖方向: Client → HTTP → Core
职责分离: 展示层 → 接口层 → 业务层
```

- **Client Layer**: 多语言、多框架的客户端 SDK
- **HTTP Layer**: REST API、WebSocket、事件流
- **Core Layer**: Agent 逻辑、Middleware、Backend
